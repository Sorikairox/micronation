...
describe('AppController', () => {
  ...
  describe('addPixel',  () => {
    it('add creation event in db', async () => {
      let addedPixelEvent = await flagService.addPixel('secondownerid', '#DDDDDD');
  ...
  describe('changePixelColor',  () => {
    ...
    it('throw error when changing color before cooldown duration ends', async () => {
      ...
      await expect(flagService.changePixelColor('ownerid', addedPixelEvent.entityId, '#FFFFFF')).rejects.toThrow(CooldownTimerHasNotEndedYet);
    });
  });
  ...
  describe('getFlag',  () => {
    it('returns last flag', async () => {
      ...
      await new Promise((r) => setTimeout(r, 1));
      let thirdPixelEvent = await flagService.addPixel('thirdowner', '#FFFFFF');
      await new Promise((r) => setTimeout(r, 1));
      await flagService.changePixelColor('thirdowner', thirdPixelEvent.entityId, '#000000');

      let flag = await flagService.getFlag();
  ...
  describe('getFlagAtDate',  () => {
    it('returns flag at given timestamp', async () => {
      ...
      await dbClientService.getDb().collection(pixelRepository.getCollectionName()).insertMany([
        {
          ...
          "createdAt":  set(date, { year: 2021, month: 7, date: 9, hours: 23, minutes: 0, seconds: 0 }),
        },
        {
          "action": "creation",
          "author": "ownerid",
          ...
          "createdAt":  set(date, { year: 2021, month: 7, date: 9, hours: 23, minutes: 10, seconds: 0 }),
        },
        {
          "action": "creation",
          "author": "ownerid",
          ...
          "createdAt":  set(date, { year: 2021, month: 7, date: 9, hours: 23, minutes: 15, seconds: 0 }),
        },
        {
          "action": "creation",
          "author": "otherownerid",
      ...
      let flag = await flagService.getFlagAtDate(set(date, { year: 2021, month: 7, date: 9, hours: 23, minutes: 10, seconds: 0 }));
